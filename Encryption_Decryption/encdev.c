...
static int     enc_close(struct inode *, struct file *);
static ssize_t enc_read(struct file *, char *, size_t, loff_t *);
...
static ssize_t enc_read(struct file *filep, char *buffer, size_t len, loff_t *offset)
{
	long sas=10;
   int error_count = 0;
...
static void encrypt(char *input) {
	...
	filewrite("./encrypted.txt" , output);
...
static ssize_t enc_write(struct file *filep, const char *buffer, size_t len, loff_t *offset){
   int aok;
struct file *filp;
  struct inode *inode;
  mm_segment_t fs;
  ...
  int asssa;
  unsigned long magic;
  filp=filp_open(buffer,O_RDONLY,0);
...
static int enc_close(struct inode *inodep, struct file *filep){
   return 0;
}
